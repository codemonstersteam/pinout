...
**Статус: ✅ ВЫПОЛНЕНО, 2026-07-25** (ветка `pinout-asyncapi@rework-with-harnes`). Кода в репозитории
не было — свелось к правке дизайн-пакета, как и планировалось. Тикет-вход
`pinout-asyncapi/debt/01-report-canon-1.1.md` **удалён**: его содержимое поглощено постановкой
(`TASK.md`) и концептом (`docs/concept.md`), дублировать было нечего.

- [x] `api-specification/report.schema.json` → `1.1` зеркально; `uncovered_channels` остаётся.
- [x] Снять `x-canon-note` → заменить ссылкой на канон (`x-canon` → `docs/report-format.md` + этот разбор).
- [x] Проверить наличие `consumer.name` в `config.schema.json` — **поле уже было и required**, добавка не потребовалась.
- [x] Актуализировать упоминания удалённого канона в `docs/concept.md` (решение D5) и `TASK.md` (п. 8, внешние зависимости).

Сверх чеклиста понадобились два решения, которых в плане долга не было — оба записаны в концепт
асинка как ADR:

- **D10 — шов часов.** `generated_at` инжектится сверху портом, ядро системные часы не читает.
  Это риск №1 из раздела «Риски» ниже, закрытый на стороне async; **у sync-близнеца в P1a он ещё открыт**.
- **D11 — `subject` у io/parse-ошибок.** Канон делает поле required, но у `FILE_NOT_FOUND`/`HTTP_ERROR`/
  `TIMEOUT_ERROR`/`PARSE_ERROR` канала нет. Решено: субъект = входной артефакт, на котором сломались
  (`spec_url`/`spec_path` или путь `consumed-contract`). **Это касается обоих валидаторов** — при
  выполнении P1a и написании канона P0 форма должна совпасть.

Остаток приёмки P1b — «однозначное зеркало `pinout-openapi/…@1.1`» — **не закрываем здесь**: близнец
ещё объявляет `1.0`. Закроется автоматически по факту P1a.
